...
def exp_mae_2(fold=1):
    """
    """
    ### Dataset Params ###
    training_dataset_path = p.data_path / "RAW2"
    validation_dataset_path = p.data_path / "RAW2"
    ...
    ### General Parameters ###
    experiment_name = f"FMC_RUNet_Patch_256_256_256_MAE_V2_Fold{fold}"
    learning_rate = 0.001
    save_step = 50
    ...
    ### Declare Model ###
    config = runet.config_256()
    model = runet.RUNet(**config)
    checkpoints = {1: 'epoch=499_general.ckpt'}
    checkpoint_path = p.checkpoints_path / f"FMC_RUNet_Patch_256_256_256_MAE_Fold{fold}" / checkpoints[int(fold)]
