...
def load_checkpoint(model_resampled, model_patch, checkpoint_path):
    state_dict = tc.load(checkpoint_path, weights_only=False)['state_dict']
    ### Model Resampled ###
...
def exp_mae_4(fold=1):
    ...
    ### General Parameters ###
    iteration = 1
    experiment_name = f"FMC_RUNet_DoubleStepDiffV2_224_06_MAE_I{iteration}_Fold{fold}"
    learning_rate = 0.001
    save_step = 50
    ...
    ### Load Checkpoint ###
    checkpoints = {1: 'epoch=349_general.ckpt'}
    checkpoint_path = p.checkpoints_path / f"FMC_RUNet_DoubleStep_224_06_MAE_I2_Fold{fold}" / checkpoints[int(fold)]
    model_resampled, model_patch = load_checkpoint(model_resampled, model_patch, checkpoint_path)
